		...
		<div class="container">
			...
			<form class="form-horizontal" action="${pageContext.request.contextPath }/video/editVideo.action?id=${video.id}" method="post">
				...
				<div class="form-group">
					...
					<div class="col-sm-10">
						<input type="text" class="form-control" name="video_title" value="${video.video_title }"  placeholder="视频标题">
					</div>
				</div>
					...
					<div class="col-sm-10">
						<select class="form-control" name="speaker_id">
							<c:forEach items="${speakerList }" var="speaker">
								<option value="${speaker.id }" ${speaker.id eq video.speaker_id ? "selected":"" } >${speaker.speaker_name }</option>
							</c:forEach>
						</select>
					</div>
				</div>
					...
					<div class="col-sm-10">
						<select class="form-control" name="course_id">
							<c:forEach items="${courseList }" var="course">
								<option value="${course.id }" ${course.id eq video.course_id ? "selected":"" } >${course.course_name }</option>
							</c:forEach>
						</select>
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-10">
						<input type="text" class="form-control" name="video_length" value="${video.video_length }" placeholder="视频时长">
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-10">
						<input type="text" class="form-control" name="video_image_url" value="${video.video_image_url }" placeholder="图片地址">
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-10">
						<input type="text" class="form-control" name="video_url" value="${video.video_url }" placeholder="视频播放地址">
					</div>
				</div>
