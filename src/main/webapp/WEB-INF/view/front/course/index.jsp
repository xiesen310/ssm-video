...
<body class="w100">
	...
	<div id="app">
		<!--banner图-->
		<div class="banner"
			style="background-image: url('<c:url value="/static/img/banner-${subject_id}.jpg"/>')"></div>

		<!--面包屑导航-->
			<div class="container mian-nav" id="navDiv">公开课 /${subject.subject_name }</div>
			<input type="hidden" id="subjectId" value="${subject_id}">
			<div class="classify">
						...
						<c:forEach items="${course.videoList }" var="video">
							...
							<div class="thum" style="background-image: url(${video.video_image_url})"></div>
								...
								<div class="classify-v-info">
									...
									<img src="<c:url value="/static/img/count.png"/>" alt="">${video.video_play_times }</span>
									<span class="duration" title="视频时长">
										<img src="<c:url value="/static/img/player.png"/>" alt="">${video.videoLengthStr }</span>
								</div>
							</li>
						</c:forEach>
					</ul>
	...
	<script type="text/javascript">
		function getVideo(id){
			...
			location.href='/video/front/video/index.action?videoId='+id+'&subjectId='+subjectId;
		}
	</script>
